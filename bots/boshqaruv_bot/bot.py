"""Boshqaruv Boti — adminlar uchun (phase 8).

Har bir amal backend'да check_store_access orqali tekshiriladi (rule 7).
Ro'yxatda bo'lmagan Telegram ID "ruxsat yo'q" javobini oladi (rule 11):
admin_ids bazада saqlanadi, .env'да EMAS — server qayta ishga tushmaydi.

Buyruqlar:
  /start            — ruxsatni tekshirish, do'konlar ro'yxati
  /mahsulot_qoshish — yangi mahsulot (ommaviy/mahfiy)
  /mahsulotlar      — do'kon mahsulotlari
  /buyurtmalar      — do'kon buyurtmalari
  /tasdiqlash <kod> — buyurtma kodini tasdiqlash
  /mahfiy_kod       — joriy kod + "Yangilash" tugmasi (rule 4, 6)
  /statistika       — do'kon statistikasi
  /top_tovarlar     — eng ko'p sotilganlar
  /promo_yaratish   — promo kod
  /yangi_dokon      — (super-admin) yangi do'kon/admin (rule 11)
"""
import sys
...
from common import BOSHQARUV_BOT_TOKEN, backend  # noqa: E402
...
async def _resolve_store(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Adminning do'konini aniqlaydi. Bittadan ko'p bo'lsa None qaytaradi va
    tanlash tugmalarini ko'rsatadi; ruxsat yo'q bo'lsa xabar beradi."""
    admin_id = update.effective_user.id
    stores = await backend.admin_my_stores(admin_id)
    if not stores:
    ...
    if len(stores) == 1:
        return stores[0]["id"]
    # Bir nechta do'kon — birinchisini standart qilamiz (soddalik uchun).
...
async def p_korinish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_id = update.effective_user.id
    r = await backend.admin_add_product(
        admin_id,
...
async def mahsulotlar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if store_id is None:
        return
    r = await backend.admin_list_products(update.effective_user.id, store_id)
    if r.status_code != 200:
...
async def buyurtmalar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if store_id is None:
        return
    r = await backend.admin_orders(update.effective_user.id, store_id)
    if r.status_code != 200:
        await update.message.reply_text(f"❌ {r.text}")
        return
    orders = r.json()
...
async def tasdiqlash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        return
    kod = context.args[0]
    r = await backend.admin_confirm_code(update.effective_user.id, kod)
    if r.status_code == 200:
        o = r.json()
        await update.message.reply_text(
        ...
        await update.message.reply_text(f"❌ {r.json().get('detail', r.text)}")
...
async def mahfiy_kod(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if store_id is None:
        return
    r = await backend.admin_get_secret_code(update.effective_user.id, store_id)
    if r.status_code != 200:
        await update.message.reply_text(f"❌ {r.text}")
        return
...
async def mahfiy_kod_refresh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    r = await backend.admin_refresh_secret_code(update.effective_user.id, store_id)
    if r.status_code != 200:
...
async def statistika(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if store_id is None:
        return
    r = await backend.admin_stats(update.effective_user.id, store_id)
    if r.status_code != 200:
        await update.message.reply_text(f"❌ {r.text}")
        return
    s = r.json()
...
async def top_tovarlar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if store_id is None:
        return
    r = await backend.admin_top_products(update.effective_user.id, store_id)
    if r.status_code != 200:
        await update.message.reply_text(f"❌ {r.text}")
        return
    top = r.json()
...
async def promo_yaratish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    r = await backend.admin_create_promo(
        update.effective_user.id,
        store_id,
        {"kod": context.args[0], "chegirma_foizi": foiz},
    )
    if r.status_code == 200:
        await update.message.reply_text(f"✅ Promo yaratildi: {context.args[0]} (-{foiz}%)")
    else:
        await update.message.reply_text(f"❌ {r.text}")


# --- Super-admin: yangi do'kon (ConversationHandler, rule 11) ---
...
async def s_admin_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        return S_ADMIN_ID
    r = await backend.super_create_store(
        update.effective_user.id,
        {
...
def main() -> None:
    if not BOSHQARUV_BOT_TOKEN:
        raise SystemExit("BOSHQARUV_BOT_TOKEN sozlanmagan (.env).")
    app = Application.builder().token(BOSHQARUV_BOT_TOKEN).build()
    ...
    print("Boshqaruv Boti ishga tushdi...")
    app.run_polling()
