...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"🏪 Adminlar kutayotgan: <b>{d['jami_adminlar_balansi']:,.0f} som</b>\n"
        f"🏦 Platforma hisobida: <b>~{d['platforma_hisobida']:,.0f} som</b>\n\n"
        f"📅 <b>Bugungi harakatlar:</b>\n"
        f"➕ To'ldirish: <b>{d['bugun_toldirish_summa']:,.0f} som</b> "
        f"({d['bugun_toldirish_soni']} ta)\n"
        f"🛍 Xaridlar: <b>{d['bugun_xarid_summa']:,.0f} som</b> "
        f"({d['bugun_xarid_soni']} ta)\n"
        f"➖ Pul yechish: <b>{d['bugun_yechish_summa']:,.0f} som</b> "
        f"({d['bugun_yechish_soni']} ta)\n"
